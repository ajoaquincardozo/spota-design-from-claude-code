...
- **Cobertura funcional:** mobile y desktop cubren los 23 CUs más auxiliares (login, register, recover, editProfile, etc.). En mobile, el `login` con foto es la única entrada de auth (Splash/Welcome se eliminó tras la unificación). En desktop, `welcome` sigue siendo landing pública y `login` tiene dos modos toggleables: `split` (foto + hero a la izquierda, form a la derecha) y `centered` (form centrado sobre crema). Cero pantallas en estado "próxima fase" en desktop tras Fase 7. Pendiente único: `editProfile` mobile.
...
- **Docs de entrega:**
  ...
  - Este `CLAUDE.md` es el brief operativo y la memoria de decisiones (D1-D17).
