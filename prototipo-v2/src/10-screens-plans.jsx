// ========================================================================
// 10-screens-plans.jsx
// ========================================================================
// CU-12, CU-13, CU-14 — Planificación grupal

// Sub-máquina del host dentro del plan grupal:
//   sin → con (vía Marketplace, transición irreversible)
//   variante post-evento agrega CTA "Calificar al host"
  ...
  if (state === 'sin') {
    return (
      <div style={{
        ...
        <div style={{ width: 38, height: 38, borderRadius: 999, background: SPOTA.c.bgAlt, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <Icon name="users" size={18} color={SPOTA.c.textSoft} />
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontFamily: SPOTA.font.ui, fontWeight: 600, fontSize: 13.5, color: SPOTA.c.text, lineHeight: 1.3 }}>¿Necesitan un host?</div>
          <p style={{ margin: '2px 0 0', fontFamily: SPOTA.font.ui, fontSize: 12, color: SPOTA.c.textSoft, lineHeight: 1.4 }}>
            Alguien que arme la experiencia y se ocupe de la logística.
          </p>
        </div>
        <Btn variant="outline" size="sm" onClick={onAdd}>Sumar</Btn>
      </div>
    );
  }
  // state === 'con'
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
        <Avatar name="Federico M." size={48} score="92" />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontFamily: SPOTA.font.ui, fontWeight: 700, fontSize: 14.5, color: SPOTA.c.text }}>Federico M. <span style={{ fontWeight: 500, color: SPOTA.c.textSoft, fontSize: 12 }}>· Fama 92</span></div>
          <p style={{ margin: '2px 0 0', fontFamily: SPOTA.font.ui, fontSize: 12.5, color: SPOTA.c.textSoft, lineHeight: 1.4 }}>
            Tour de cafés de especialidad por Palermo · 3 hs
          </p>
        </div>
        <span style={{ fontFamily: SPOTA.font.ui, fontWeight: 700, fontSize: 14, color: SPOTA.c.text, whiteSpace: 'nowrap' }}>$12.500</span>
      </div>
      {postEvent && (
        <button onClick={onRate} style={{
          marginTop: 12, width: '100%', padding: '8px 12px', borderRadius: 10,
          background: SPOTA.c.accentSoft, border: 'none', cursor: 'pointer',
          fontFamily: SPOTA.font.ui, fontWeight: 600, fontSize: 13, color: '#7A5A12',
          display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
        }}>
          <Icon name="star" size={14} color={SPOTA.c.accent} /> Calificar al host
        </button>
      )}
    </div>
  );
...
const ScreenCreatePlan = ({ nav }) => {
  ...
  const [planDate, setPlanDate] = React.useState('17/05/2026');
  ...
  const onCopyLink = () => { setCopyToast(true); setTimeout(() => setCopyToast(false), 2200); };
  const onAddInvitee = () => {
    const t = inviteQuery.trim();
    if (!t || invited.includes(t)) return;
    setInvited([...invited, t]);
    setInviteQuery('');
  };
  const total = 3;
        ...
        {step === 2 && (
          <>
            ...
            <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
              <Field label="Fecha tentativa (DD/MM/AAAA)" value={planDate} onChange={setPlanDate} icon="calendar" />
              <Field label="Horario (HH:MM)" value={planTime} onChange={setPlanTime} icon="clock" />
            </div>
          </>
        )}
        {step === 3 && (
          <>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
              <div style={{ flex: 1 }}>
                <Field label="" placeholder="Buscar por nombre o email…" icon="search" value={inviteQuery} onChange={setInviteQuery} />
              </div>
              <Btn variant="outline" size="md" icon="plus" disabled={!inviteQuery.trim()} onClick={onAddInvitee}>Sumar</Btn>
            </div>
            <div style={{ marginTop: 16, padding: 14, borderRadius: 14, background: SPOTA.c.primarySoft, border: `1px dashed ${SPOTA.c.primary}`, display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <Btn variant="primary" size="sm" onClick={onCopyLink}>{copyToast ? 'Copiado ✓' : 'Copiar'}</Btn>
            </div>
