...
const Icon = ({ name, size = 24, color = 'currentColor', strokeWidth = 1.75, fill = 'none' }) => {
  ...
  switch (name) {
    ...
    case 'mask': return <svg {...p}><path d="M4 6c0-1 1-2 2-2h12c1 0 2 1 2 2v6c0 5-4 8-8 8s-8-3-8-8z"/><circle cx="9" cy="11" r="1.2" fill={color}/><circle cx="15" cy="11" r="1.2" fill={color}/><path d="M9 15c1 1 2 1.5 3 1.5s2-.5 3-1.5"/></svg>;
    case 'yoga': return <svg {...p}><circle cx="12" cy="5" r="2"/><path d="M12 7v6M5 21c0-3 3-6 7-6s7 3 7 6M8 11l4 2 4-2"/></svg>;
    case 'building': return <svg {...p}><rect x="4" y="3" width="16" height="18" rx="1"/><path d="M9 7h2M13 7h2M9 11h2M13 11h2M9 15h2M13 15h2M10 21v-3h4v3"/></svg>;
    case 'wineglass': return <svg {...p}><path d="M8 3h8l-1 6a3 3 0 0 1-6 0zM12 12v9M9 21h6"/></svg>;
    case 'fire': return <svg {...p}><path d="M12 3c0 4-5 5-5 10a5 5 0 0 0 10 0c0-2-1-3-2-4 0 2-1 3-2 3 1-3-1-7-1-9z"/></svg>;
    case 'moon': return <svg {...p}><path d="M21 12.8A9 9 0 1 1 11.2 3a7 7 0 0 0 9.8 9.8z"/></svg>;
    case 'twoUsers': return <svg {...p}><circle cx="9" cy="9" r="3"/><circle cx="15" cy="9" r="3"/><path d="M3 20c0-3 3-5 6-5s6 2 6 5M15 15c2.5 0 6 1.5 6 5"/></svg>;
    case 'family': return <svg {...p}><circle cx="7" cy="7" r="2.5"/><circle cx="17" cy="7" r="2.5"/><circle cx="12" cy="11" r="1.8"/><path d="M3 19c0-2.5 2-4 4-4s4 1.5 4 4M13 19c0-2.5 2-4 4-4s4 1.5 4 4M10 21c0-1.5 1-2.5 2-2.5s2 1 2 2.5"/></svg>;
    case 'walk': return <svg {...p}><circle cx="13" cy="4" r="2"/><path d="M9 21l3-7 3 4 3-1M5 13l4-3 3 4"/></svg>;
...
// ─────────────────────────────────────────────────────────────
const FamaScore = ({ score = 4.7, count = 256, affinity = 92, variant = 'classic', size = 'sm', onDark = false }) => {
  if (variant === 'ring') {
...
// ─────────────────────────────────────────────────────────────
// AffineRow — visualiza Fama Score predictivo en clave humana:
// chip cualitativo de match + mini-avatares de reseñadores afines.
// Es la traducción de "afinidad %" a algo lecible en card chica.
// ─────────────────────────────────────────────────────────────
const AffineRow = ({ place }) => {
  const reviewers = (window.AFFINE_REVIEWERS && window.AFFINE_REVIEWERS[place.id]) || [];
  const aff = window.affinityLabel(place.affinity);
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
      {aff && (
        <span style={{
          ...
          background: aff.tone === 'primary' ? 'rgba(47, 111, 94, 0.10)' : SPOTA.c.bgAlt || '#F5EDE0',
      ...
      {reviewers.length > 0 && (
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, minWidth: 0 }}>
          ...
          <span style={{ fontFamily: SPOTA.font.ui, fontSize: 11.5, color: SPOTA.c.textSoft, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
...
// Cada chip lleva un microprefijo en Fraunces italic (la dimensión) y el
// valor en DM Sans semibold. Es informativo, no editable.
// ─────────────────────────────────────────────────────────────
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
        {present.map(d => (
          <span key={d.key} style={{
            ...
            fontFamily: SPOTA.font.ui, fontSize: 12.5,
          }}>
...
// ─────────────────────────────────────────────────────────────
const SearchBar = ({ onClick, expanded, onSubmit, value, onChange, placeholders = [], hero = false }) => {
  ...
  // Modo hero: el input pasa a ser bloque protagónico de la pantalla
  // (alto generoso, fondo blanco, borde primary, shadow marcada).
  const styles = hero ? {
    height: 64, padding: '0 20px',
    background: '#FFFFFF',
    border: `1.5px solid ${SPOTA.c.primary}`,
    boxShadow: '0 8px 22px rgba(47, 111, 94, 0.12), 0 1px 3px rgba(47, 111, 94, 0.08)',
    iconColor: SPOTA.c.primary,
    iconSize: 22,
    fontSize: 16,
    phHeight: 24,
  } : {
  ...
  return (
    <div onClick={onClick} style={{
      ...
      padding: styles.padding, height: styles.height,
      background: styles.background, borderRadius: SPOTA.radius.pill,
      border: styles.border, boxShadow: styles.boxShadow,
      cursor: 'text',
    }}>
      <Icon name="search" size={styles.iconSize} color={styles.iconColor} strokeWidth={hero ? 2.2 : 2} />
      {expanded ? (
        <>
          {/* P3-32: límite 200 chars con contador discreto a la derecha. */}
          <input autoFocus value={value} onChange={(e) => onChange && onChange(e.target.value.slice(0, 200))}
            onKeyDown={(e) => { if (e.key === 'Enter' && onSubmit) onSubmit(); }}
            maxLength={200}
            placeholder={ph}
            style={{
              flex: 1, border: 'none', outline: 'none', background: 'transparent',
              fontFamily: SPOTA.font.ui, fontSize: styles.fontSize, color: SPOTA.c.text,
              minWidth: 0,
            }} />
          {hero && value && value.length > 0 && (
            <span style={{ fontFamily: SPOTA.font.ui, fontSize: 11, color: value.length >= 180 ? SPOTA.c.secondary : SPOTA.c.textSoft, fontVariantNumeric: 'tabular-nums', flexShrink: 0 }}>
              {value.length}/200
            </span>
          )}
        </>
      ) : (
        <div style={{ flex: 1, position: 'relative', height: styles.phHeight, overflow: 'hidden' }}>
          {placeholders.map((p, i) => (
            <span key={i} style={{
              position: 'absolute', inset: 0, display: 'flex', alignItems: 'center',
              fontFamily: SPOTA.font.ui, fontSize: styles.fontSize - 0.5, color: SPOTA.c.textSoft,
              opacity: i === phIdx ? 1 : 0,
...
// Input field
// ─────────────────────────────────────────────────────────────
const Field = ({ label, type = 'text', value, onChange, placeholder, icon, hint, error, onDark = false }) => {
  const [focus, setFocus] = React.useState(false);
  const [show, setShow] = React.useState(false);
  const isPwd = type === 'password';
  return (
    <label style={{ display: 'block', flex: 1, minWidth: 0 }}>
      {label && <div style={{ fontFamily: SPOTA.font.ui, fontWeight: 600, fontSize: 13, color: onDark ? '#fff' : SPOTA.c.text, marginBottom: 6 }}>{label}</div>}
      <div style={{
        display: 'flex', alignItems: 'center', gap: 10,
        height: 50, padding: '0 14px', minWidth: 0,
        ...
        <input
          ...
          style={{ flex: 1, minWidth: 0, width: 0, border: 'none', outline: 'none', background: 'transparent',
...
Object.assign(window, {
  ...
  IntentChips, INTENT_DIMENSIONS, AffineRow,
});
