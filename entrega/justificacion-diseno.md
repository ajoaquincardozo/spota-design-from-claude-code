...
15. **Concierge-first en Discover (mobile y desktop).** El brief, el MVP y la propuesta tecnológica enuncian la misma tesis: Spota es un *concierge*, no un buscador. Interpreta intención. La primera versión del prototipo no honraba esa tesis: el home pusheaba un feed de 12 cards (Hero + "Para vos" + "Cerca tuyo" + "Más para descubrir") sin que el usuario hubiera expresado nada, el input era un control más entre los chips de categoría, y los resultados de búsqueda mostraban chips hardcoded `['Palermo','Tranquilo','Café','Abre ahora']` que ignoraban la query real. Visualmente Spota era TripAdvisor; la promesa de "interpreta intención" vivía solo en el placeholder.

   La decisión es invertir el modelo. El home queda con saludo + input hero + atajos discretos por categoría + ejemplos clicables. **Sin recomendaciones por default**. Las cards aparecen recién en `searchResults`, que ahora es la pantalla pivote del descubrimiento. El input se vuelve protagonista por tamaño (alto 64 px mobile, 72 px desktop), color (fondo blanco vs `surface` crema), borde (1.5 px primary) y shadow tinted con verde petróleo — los cuatro drivers de jerarquía visual juegan a su favor, no en contra.

   **Capas técnicas que sustentan la promesa.** Tres componentes nuevos materializan la tesis:

   | Pieza | Qué hace | Por qué importa |
   |---|---|---|
   | `interpretQuery(text)` | Parser heurístico que extrae dimensiones de la query libre: `ambiente`, `compania`, `momento`, `categoria`, `zona`. | El concierge no puede ser una promesa textual: el sistema tiene que demostrar que *leyó* lo que escribió el usuario. Para esto el parser es heurístico, mock; la decisión de diseño es independiente del modelo (puede pasar a LLM real sin tocar la UI). |
   | `IntentChips` | Renderiza las dimensiones interpretadas como chips visibles arriba de los resultados ("ambiente · tranquilo", "con amiga", "en Palermo"). Microprefijos en Fraunces italic, valores en DM Sans semibold. | Es la traducción visual de "te entendí". Reemplaza los chips hardcoded; lo que ves es lo que el sistema cruzó con tu perfil. |
   | `AffineRow` (en `PlaceCard`) | Chip cualitativo de match ("Alto match", "Buen match") + 3 mini-avatares apilados con la frase "gente como vos". | Es la traducción humana del Fama Score predictivo. La afinidad numérica (94 %) sigue existiendo en el dato, pero la card chica no la pide; pide razón. "Alto match · gente como vos" es razón, no número. |

   **Loading narrativo (750 ms).** Entre el submit y los resultados se inserta una pausa con tres puntos pulsando y el copy *"Leyendo tus gustos…"* + *"Cruzando ambiente · con amiga · sábado · Palermo con tu Fama Score"*. La duración es deliberada: menos de 500 ms el usuario no la ve, más de 1 s molesta. 750 ms comunica que hay un cerebro detrás sin penalizar la rapidez. Si después la inferencia es real y tarda más, el copy ya está en su lugar. Si tarda menos, el delay artificial mantiene la expectativa narrativa.

   **Refinable, no contractual.** El header de `searchResults` muestra la query como input editable: tipear y dar Enter (o el botón "Refinar") re-ejecuta la búsqueda con loading nuevo. Esto resuelve el problema previo del header read-only que cortaba la query con ellipsis y obligaba a volver al home para corregir. Conceptualmente es coherente con el diálogo: el concierge te entendió, vos decís "no, eso no" y refrasea.

   **Por qué se eliminó el buscador permanente del TopNav desktop.** Después de hacer el input hero del home, el campo del navbar (placeholder "Buscar lugares, colecciones, hosts…") competía contra el protagonista. La regla es: si el input es protagonista del home, no puede haber otro input persistente que dispute esa atención. Buscar deja de ser "tener un campo siempre visible" y pasa a ser "ir al home y expresar intención", como en Spotify, Instagram o Netflix. Si en el futuro hace falta un atajo global, lo natural es un command palette estilo Linear (cmd+K), no un input chico siempre encendido.

   **Por qué D12 sigue valiendo (con cambio de scope).** D12 estableció "lista + mapa simultáneos, no toggle" para Discover desktop. Como Discover ya no tiene feed, esa decisión migró a `searchResults`: ahí siguen las dos columnas (lista 60 % + mapa 40 % sticky) porque ahí están los lugares que ameritan ser georreferenciados. La decisión es la misma; la pantalla cambió.

   **Trade-off explícito.** Se sacrifica la retención pasiva del usuario que "no sabe qué quiere y scrollea". Apps consumer modernas (Instagram, Spotify, Netflix) construyen su valor sobre ese scroll. Spota dice no: si entrás a Spota es porque querés salir, y la fatiga de decisión se resuelve declarando intención, no recibiendo más estímulos. Es una apuesta consistente con el problema central que enuncia el brief — "las personas no carecen de opciones, carecen de certeza" — y obliga a que el concierge demuestre valor en cada interacción, no a esconderlo detrás de un feed.
